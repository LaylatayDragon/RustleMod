...
public class CalfEntity extends Animal implements IAnimatable {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(RustleModEntities.CALF.get(), 20, 4, 4));
	}

	public CalfEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RustleModEntities.CALF.get(), world);
	}

	public CalfEntity(EntityType<CalfEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(RustleModEntities.CALF.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(CalfEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
	@Override
	...
	public float getPercentAdult()
	{
		return age/(float)adultAge;
	}


	@Override
	public SoundEvent getAmbientSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("rustle:calf_hum"));
	}
